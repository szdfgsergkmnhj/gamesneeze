...
void Hooks::OverrideView::hook(void* thisptr, ViewSetup* setup) {
    if (Interfaces::engine->IsInGame() && Globals::localPlayer && Globals::localPlayer->health() > 0 && !Globals::localPlayer->scoped()) {
        ...
        if (Menu::CustomWidgets::isKeyDown(CONFIGINT("Visuals>World>World>Third Person Key"))){
            QAngle viewAngles;
            ...
            Vector camOff = Vector(cos(DEG2RAD(viewAngles.y)) * CONFIGINT("Visuals>World>World>Third Person Distance"),sin(DEG2RAD(viewAngles.y)) * CONFIGINT("Visuals>World>World>Third Person Distance"),sin(DEG2RAD(-viewAngles.x)) * CONFIGINT("Visuals>World>World>Third Person Distance"));
            ...
            Interfaces::input->m_vecCameraOffset = Vector(viewAngles.x, viewAngles.y, CONFIGINT("Visuals>World>World>Third Person Distance") * ((tr.fraction < 1.0f) ? tr.fraction : 1.0f) );
